...
// NEW: nếu có audioUrl thì dùng audio mp3 từ URL, bỏ TTS và bỏ subtitle
...
async function downloadImage(url, dest, retries = 3) {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      lastError = err;
      console.warn(
        `[downloadImage] Attempt ${attempt}/${retries} failed: ${err.message}`,
      );
      if (attempt < retries)
  ...
  throw new Error(
    `Download failed after ${retries} attempts (${url}): ${lastError.message}`,
  );
}

async function downloadAudio(url, dest, retries = 3) {
  let lastError;
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (!fs.existsSync(dest)) {
        throw new Error("Audio file not created after download");
      }
      ...
      lastError = err;
      console.warn(
        `[downloadAudio] Attempt ${attempt}/${retries} failed: ${err.message}`,
      );
      if (attempt < retries) {
        await new Promise((r) => setTimeout(r, 2000 * attempt));
        if (fs.existsSync(dest)) {
  ...
  throw new Error(
    `Audio download failed after ${retries} attempts (${url}): ${lastError.message}`,
  );
}
...
async function generateAudioWithRetry(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (!fs.existsSync(audioPath))
        throw new Error("Audio file not created after TTS");
      return result;
    } catch (err) {
      lastError = err;
      console.warn(
        `[${sessionId}] ⚠️ TTS attempt ${attempt} failed: ${err.message}`,
      );
      if (attempt < maxRetries) {
        await new Promise((r) =>
          setTimeout(r, Math.min(2000 * attempt, 10000)),
        );
        if (fs.existsSync(audioPath)) {
          try {
  ...
  throw new Error(
    `TTS failed after ${maxRetries} attempts: ${lastError.message}`,
  );
}
...
async function renderTitleClip(title, duration, outPath, sessionId) {
  ...
  const startY = Math.max(
    TITLE_MARGIN_T,
    Math.floor((TITLE_H - totalTextH) / 2),
  );
  ...
  console.log(
    `[${sessionId}] 🏷️ Title (centered): ${lines.length} line(s) → "${lines.join(" / ")}"`,
  );
...
async function renderSlideshowClip(imgPaths, duration, outPath, sessionId) {
  ...
  const slideList = Array.from(
    { length: totalSlides },
    (_, i) => imgPaths[i % imgPaths.length],
  );

  console.log(
    `[${sessionId}] 🖼️ Slideshow blur-bg: ${imgPaths.length} imgs → ${totalSlides} slides × ${SEC_PER_SLIDE}s`,
  );
  ...
  for (let i = 1; i < segPaths.length; i++) {
    ...
    filterParts.push(
      `${prev}[${i}:v]xfade=transition=fade:duration=${TRANSITION}:offset=${offset}${label}`,
    );
    prev = `[xf${i}]`;
  }
...
async function renderSubtitleBand(duration, srtPath, outPath, sessionId) {
  ...
  console.log(
    `[${sessionId}] 💬 Subtitle band on black bg (${W}x${SUB_BAND_H})`,
  );
...
async function renderBgVideoLoop(duration, outPath, sessionId) {
  ...
  console.log(
    `[${sessionId}] 🎥 BG 16:9: ${bgDuration.toFixed(2)}s × ${loopCount} loops → ${W}x${BG_H}`,
  );
...
// ═══════════════════════════════════════════════════════════════════════════════
async function stackFourLayers(
  titlePath,
  imagePath,
  subBandPath,
  bgPath,
...
async function processVideo(
  ...
  try {
    ...
    onProgress({
      ...
      step: "audio",
      message: audioUrl
        ? "Đang tải audio từ audioUrl..."
        : "Đang tạo giọng đọc TTS...",
    });

    if (audioUrl) {
      await downloadAudio(audioUrl, audioPath);
      audioSource = "external-audio-url";
      console.log(`[${jobId}] 🎧 Audio downloaded from audioUrl`);
    } else {
      const ttsResult = await generateAudioWithRetry(
        content,
        audioPath,
        jobId,
        5,
    ...
    onProgress({
      progress: 15,
      step: "images",
      message: `Đang tải ${imageUrls.length} ảnh...`,
    });
    ...
    onProgress({
      progress: 28,
      step: "render",
      message: "Đang render title, slideshow, subtitle band, bg...",
    });
    ...
    return {
      ...
      metadata: {
        ...
        layout: [
          ...
          `Subtitle Band ${W}x${SUB_BAND_H} (black bg, yellow BebasNeue / empty when no SRT)`,
          `BG Video ${W}x${BG_H} 16:9`,
        ].join(" | "),
        resolution: `${W}x${TOTAL_H}`,
        mode: audioUrl
          ? localImgPaths.length === 1
            ? "single-blurbg-external-audio"
            : `slideshow-blurbg-external-audio-${localImgPaths.length}imgs`
          : localImgPaths.length === 1
...
async function runJob(jobId) {
  ...
  const { content, title, images, language, audioUrl } = job.payload;
...
router.post("/", async (req, res) => {
  const { content, title, images, language, audioUrl } = req.body;

  if (
    !audioUrl &&
    (!content || typeof content !== "string" || content.trim().length < 10)
  ) {
    return res.status(400).json({
      success: false,
      error:
        "content (string, min 10 chars) required when audioUrl is not provided",
    });
  }
  ...
  if (!title || typeof title !== "string" || title.trim().length < 2) {
    return res
      .status(400)
      .json({ success: false, error: "title (string) required" });
  }
  ...
  if (imageUrls.length === 0) {
    return res.status(400).json({
      success: false,
      error: "images (string or array of URLs) required",
    });
  }
  ...
  if (imageUrls.length > 20) {
    return res
      .status(400)
      .json({ success: false, error: "Max 20 images per request" });
  }

  if (
    audioUrl &&
    (typeof audioUrl !== "string" || !/^https?:\/\//i.test(audioUrl))
  ) {
    return res.status(400).json({
      success: false,
      error: "audioUrl must be a valid http/https URL",
    });
  }
  ...
  console.log(
    `║ 🖼️ Images: ${imageUrls.length} | Layout: ${W}x${TITLE_H}+${IMAGE_H}+${SUB_BAND_H}+${BG_H}`,
  );
  console.log(`║ 🔊 Audio Source: ${audioUrl ? "audioUrl" : "TTS"}`);
  console.log(`╚══════════════════════════════════════════════════════╝`);
